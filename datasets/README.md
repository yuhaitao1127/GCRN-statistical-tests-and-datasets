...
CF and CF* share the same 300 tasks and gold labels but have different worker pools and annotation sets. Both raw datasets remain available here. The corrected statistical tests use their equal-weight per-seed composite as one independent block; see the repository README.
...
Dataset dimensions and name mappings are recorded in `datasets/DATASET_METADATA.csv`.
